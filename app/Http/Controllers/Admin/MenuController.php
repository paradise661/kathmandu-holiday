...
class MenuController extends Controller
{
    public function index()
    {
        ...
        return view('admin.menu.index', ['posts' => News::where('status', 1)->get(), 'pages' => Page::where('status', 1)->get(), 'packages' => Package::where('status', 1)->get(), 'blogcategorys' => Blogcategory::where('status', 1)->where('parent_id', 0)->get(), 'activities' => Activity::where('status', 1)->where('parent_id', 0)->get(), 'destinations' => Destination::where('status', 1)->where('parent_id', 0)->get(), 'services' => Services::where('status', 1)->get(), 'menus' => MenuLocation::all(), 'desiredMenu' => $desiredMenu, 'menuitems' => $menuitems]);
    }
    ...
    public function addDestinationToMenu(Request $request)
    {
        ...
        if ($menu->content == '') {
            foreach ($ids as $id) {
                ...
                $data['slug'] = '/destinations/' . Destination::where('id', $id)->value('slug');
            ...
            foreach ($ids as $id) {
                ...
                $array['slug'] = '/destinations/' . Destination::where('id', $id)->value('slug');
                $array['name'] = NULL;
                $array['type'] = 'destinations';
                $array['target'] = NULL;
                $array['id'] = Menu::where('slug', $array['slug'])->where('name', $array['name'])->where('type', $array['type'])->orderby('id', 'DESC')->value('id');
                $array['children'] = [[]];
                array_push($olddata[0], $array);
    ...
    public function addCategoryToMenu(Request $request)
    {
        ...
        if ($menu->content == '') {
            foreach ($ids as $id) {
                $data['title'] = Activity::where('id', $id)->value('name');
                $data['slug'] = '/activities/' . Activity::where('id', $id)->value('slug');
            ...
            foreach ($ids as $id) {
                $array['title'] = Activity::where('id', $id)->value('name');
                $array['slug'] = '/activities/' . Activity::where('id', $id)->value('slug');
                $array['name'] = NULL;
                $array['type'] = 'activities';
                $array['target'] = NULL;
                $array['id'] = Menu::where('slug', $array['slug'])->where('name', $array['name'])->where('type', $array['type'])->orderby('id', 'DESC')->value('id');
                $array['children'] = [[]];
                array_push($olddata[0], $array);
